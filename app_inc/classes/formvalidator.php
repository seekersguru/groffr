...
class FormValidator 
{
	...
	function validate_req($input_value, &$default_error_message,$variable_name)
	{
	  $bret = true;
		...
		/*if(ereg("[^_\.0-9a-zA-Z- ]",$input_value))
		{
		$bret=false;
			return $bret;
		}*/
	...
	{ 
		...
		preg_match($pattern1, $pass, $matches1);
	...
	function ValidateCommand($command,$command_value,$input_value,&$default_error_message,$variable_name,$formvariables)
	{
		...
		switch($command)
		{
			case 'req':
						{
							$bret = $this->validate_req(trim(strip_tags($input_value)), $default_error_message,$variable_name);
							break;
						}
			...
			case 'Alphapass':
						{
							if(isset($input_value) && strlen($input_value)>0)
							{
								$bret= $this->validate_pass($input_value);
								if(false == $bret)
								{
									$default_error_message = E_VAL_EMAIL_CHECK_FAILED;
								}
